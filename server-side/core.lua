...
-----------------------------------------------------------------------------------------------------------------------------------------
Detroit = {}
Tunnel.bindInterface("crafting", Detroit)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VERIFY
-----------------------------------------------------------------------------------------------------------------------------------------
function Detroit.Verify()
	local source = source
...
-- PERMISSION
-----------------------------------------------------------------------------------------------------------------------------------------
-- function Detroit.Permission(Type)
--     local source = source
--     local Passport = vRP.Passport(source)
--     if Passport then
...
function Detroit.Permission(Type)
    ...
    local Passport = vRP.Passport(source)
    if Passport then
        local playerLevel = GetPlayerLevel(source) -- Obtenha o nível do jogador

        if List[Type]["List"] then
            if List[Type]["perm"] ~= nil then
                if vRP.HasGroup(Passport,List[Type]["perm"]) or playerLevel >= List[Type]["List"][Item]["requiredLevel"] then
                    return true
                end
            else
                return true
...
-- FUNCTIONCRAFTING
-----------------------------------------------------------------------------------------------------------------------------------------
-- function Detroit.Crafting(Type, Item, Amount)
--     local source = source
...
function Detroit.Crafting(Type, Item, Amount)
    ...
    if Passport then
        ...
        if List[Type]["List"][Item] then
            ...
            if playerLevel >= List[Type]["List"][Item]["requiredLevel"] then
                if vRP.MaxItens(Passport, Item, List[Type]["List"][Item]["amount"] * Amount) then
                    TriggerClientEvent("Notify", source, "amarelo", "Limite atingido.", "Atenção", 5000)
                    TriggerClientEvent("crafting:Update", source, "requestCrafting")
                    return
                ...
                TriggerClientEvent("Notify", source, "amarelo", "Você não tem o nível necessário para criar este item.")
            end
        end
...
-- OWNED
-----------------------------------------------------------------------------------------------------------------------------------------
function Detroit.Owned(Liste, Item)
    local Owned = {}
    local source = source
    local Passport = vRP.Passport(source)
...
-- FUNCTIONDESTROY
-----------------------------------------------------------------------------------------------------------------------------------------
function Detroit.functionDestroy(Item, Type, Amount, Slot)
    local source = source
    local Amount = parseInt(Amount)
    local Passport = vRP.Passport(source)
    if Passport then
